...
class CustomServiceManager(BaseServiceManager):
    ...
    def _get_config(kind: str) -> dict:
        ...
        if kind not in CustomServiceManager.SUPPORTED_KINDS:
            raise ValueError(
                f"Unsupported training job type: {kind}. "
                f"Supported types: {', '.join(CustomServiceManager.SUPPORTED_KINDS)}"
            )
        ...
        config = {
            "replica_specs_key": kind.lower().replace("job", "") + "ReplicaSpecs",
    ...
    def __init__(
        ...
        for supported_kind in CustomServiceManager.SUPPORTED_KINDS:
