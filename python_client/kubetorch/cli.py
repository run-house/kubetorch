    ...
    try:
        ...
        for svc in unified_services:
            ...
            if kind == "trainjob":
                ...
                pod_map[name] = [
                    pod
                    for pod in jobset_pods
                    if pod.get("metadata", {}).get("labels", {}).get("jobset.sigs.k8s.io/jobset-name") == name
                ]
            else:
                ...
                pod_map[name] = [
                    pod
                    for pod in kt_pods
                    if pod.get("metadata", {}).get("labels", {}).get(serving_constants.KT_SERVICE_LABEL) == name
                ]
