...
def profiler_test_helper(profiler_output_path: Path, file_suffix: str = "svg"):
    ...
    assert str(profiler_output_path).endswith(file_suffix)
...
@pytest.mark.asyncio
@pytest.mark.gpu_test
def test_profiling_pytorch_table_output(remote_profiling_torch_fn, caplog):
    pytorch_profiler_configs = [
        ProfilerConfig(profiler_type="pytorch", output_format="table"),  # table output with default values
        ProfilerConfig(profiler_type="pytorch", output_format="table", table_sort_by="count"),  # provide sort by
        ProfilerConfig(
            profiler_type="pytorch", output_format="table", group_by_input_shape=True
        ),  # provide group_by_input_shape
        ProfilerConfig(
            profiler_type="pytorch", output_format="table", group_by_stack_n=5
        ),  # provide group_by_input_shape
        ProfilerConfig(
            profiler_type="pytorch",
            output_format="table",
            table_sort_by="cuda_memory_usage",
            group_by_stack_n=5,
            group_by_input_shape=True,
        ),  # multiple config args
    ]
    for profiler_config in pytorch_profiler_configs:
        iterations = random.randint(5, 10)
        res, table = remote_profiling_torch_fn(num_iterations=iterations, profiler=profiler_config)
...
@pytest.mark.asyncio
@pytest.mark.gpu_test
def test_profiling_pytorch_output_types_fn(remote_profiling_torch_fn, caplog):
    valid_pytorch_outputs = ["chrometrace", "memory_timeline", "stacks"]

    with tempfile.TemporaryDirectory(suffix=dir_suffix) as output_tmpdir:
        for output in valid_pytorch_outputs:
            random_file_name = f"{uuid.uuid4().hex[:8]}"
            pytorch_profiler_config = ProfilerConfig(
                profiler_type="pytorch",
                output_path=output_tmpdir,
                output_filename=random_file_name,
            ...
            res = remote_profiling_torch_fn(num_iterations=8, profiler=pytorch_profiler_config)
            assert res == "matrix_dot_torch ran successfully!"

            expected_output_file_path = Path(output_tmpdir) / Path(f"{random_file_name}.json")
            profiler_test_helper(profiler_output_path=expected_output_file_path, file_suffix="json")
...
@pytest.mark.asyncio
@pytest.mark.gpu_test
def test_profiling_pytorch_memory_timeline_cls(remote_profiling_torch_cls, caplog):
    valid_timeline_outputs = ["html", "json", "json_zip", "raw"]
    timeline_expected_file_suffix = {"html": "html", "json": "json", "json_zip": "json.gz", "raw": "raw.json.gz"}

    with tempfile.TemporaryDirectory(suffix=dir_suffix) as output_tmpdir:
        for timeline_output in valid_timeline_outputs:
            random_file_name = f"{uuid.uuid4().hex[:8]}"
            pytorch_profiler_config = ProfilerConfig(
                profiler_type="pytorch",
                output_path=output_tmpdir,
                output_filename=random_file_name,
                output_format="memory_timeline",
                memory_timeline_output_type=timeline_output,
            )

            res = remote_profiling_torch_cls.dot_torch(profiler=pytorch_profiler_config)
            assert res == "dot_torch in Matrix_CPU class ran successfully!"

            file_suffix = timeline_expected_file_suffix.get(timeline_output)
            expected_output_file_path = Path(output_tmpdir) / Path(f"{random_file_name}.{file_suffix}")
            profiler_test_helper(profiler_output_path=expected_output_file_path, file_suffix=file_suffix)


@pytest.mark.level("unit")
@pytest.mark.asyncio
def test_profiling_invalid_config_setup(caplog):
    from kubetorch.servers.http.constants import (
    ...
    # case 4: invalid pytorch table_sort_by
    invalid_sort_key = "invalid_sort_key"
    with pytest.raises(ValueError) as raised_exception:
        ProfilerConfig(profiler_type="pytorch", output_format="table", table_sort_by=invalid_sort_key)

    assert f"Invalid table_sort_by for Pytorch profiler: '{invalid_sort_key}'" in str(raised_exception.value)
    assert f"Must be one of {SUPPORTED_PYTORCH_TABLE_SORT_KEYS}" in str(raised_exception.value)
